...
export function RegionEditor(props: {
  ...
  const recognize = () => task('正在识别文字、Logo 和物体…', async () => { const result = await regionTask<{ regions: DesignRegion[] }>({ action: 'detect', image: source }); if (alive.current) { saveRegions(result.regions); if (!result.regions.length) setError('没有识别到清晰区域，可以手动框选'); } });
  ...
  return <div className="region-editor">
    ...
    <aside className="region-panel">
      ...
      {props.pending && !busy && !preview && <div className="studio-resume"><p>已提交编辑任务，继续查询不会重新生成。</p><button onClick={() => generate(props.pending)}>继续查询结果</button><button onClick={() => props.onPending(undefined)}>放弃此任务</button></div>}
