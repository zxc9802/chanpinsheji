...
export function QuickDesignStudio() {
  ...
  async function upload(file: File, type: 'reference' | 'document') {
    ...
    try {
      if (type === 'reference') { const dataUrl = await prepareUpload(file); if (active(id)) patch({ reference: { name: file.name, dataUrl, mode: studio.reference ? studio.reference.mode || 'structure' : 'style' }, ...resetDraft }); }
        ...
        if (active(id)) { ctx.importParsedBrief(groundedBrief(result.brief, result.fieldSources), result.fieldSources); patch({ documentName: file.name, ...resetDraft }); setNotice('已保留文档中的信息；未提供的内容留空，可以直接开始设计。'); }
      }
    ...
    try {
      await generateStudioConcepts(brief, initial, {
        ...
        dependencies: (context, checkpoint) => {
          ...
          const creativeContext = { brief, styleHint: studio.styleHint, designHint: context.concepts?.find(c => c.id === context.activeConceptId)?.designHint, referenceMode: context.reference ? context.reference.mode || 'structure' as const : undefined };
          return {
            ...
            copy: async (b, hint) => { const options = await copyGenerator.generate({ brief: b, toneHint: hint, allowPartial: true }); if (!options[0]) throw new Error('文案没有返回有效内容'); return options[0]; },
  ...
  return <div className="quick-studio">
    ...
    {setupVisible && <section className="studio-setup">
      ...
      {(studio.documentName || brief.brand.name || brief.product.name) && <div className="studio-facts"><label>品牌名（可不填）<input disabled={locked} value={brief.brand.name} onChange={e => { ctx.setBrief({ ...brief, brand: { ...brief.brand, name: e.target.value } }); patch({ ...resetDraft }); }} /></label><label>产品名（可不填）<input disabled={locked} value={brief.product.name} onChange={e => { ctx.setBrief({ ...brief, product: { ...brief.product, name: e.target.value } }); patch({ ...resetDraft }); }} /></label><div><small>已提取的产品信息</small><p>{[brief.product.category, ...brief.product.coreSellingPoints.map(p => p.point)].filter(Boolean).join(' · ') || (brief.additionalInfo ? '已保留补充资料' : '文档未提供更多产品事实')} <Link href="/workflow/1">查看与修改全部资料 ↗</Link></p>{brief.additionalInfo && <p style={{ whiteSpace: "pre-wrap" }}>{brief.additionalInfo}</p>}</div></div>}
    ...
    {studio.direction && <details className="studio-copy"><summary>当前方案设计方向 · {studio.direction.plan.concept || selectedConcept.name}</summary><p>{studio.direction.plan.referenceInsights}</p><div className="studio-copy-fields">{[['配色', studio.direction.plan.palette.map(p => `${p.color}：${p.role}`).join('；')], ['字体与信息层级', studio.direction.plan.typography], ['Logo', studio.direction.plan.logo], ['内包装', studio.direction.plan.product], ['外包装', studio.direction.plan.packaging], ['其他设计建议', studio.direction.plan.notes]].filter(([, value]) => value).map(([label, value]) => <article key={label}><small>{label}</small><p>{value}</p></article>)}</div></details>}
