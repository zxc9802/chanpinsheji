...
import type { AssetKind, StudioState } from '@/types/studio';
...
export function QuickDesignStudio() {
  ...
  async function upload(file: File, type: 'reference' | 'document') {
    if (busy) return;
    const id = brief.projectId; setBusy(type === 'reference' ? '正在读取瓶身参考图…' : '正在提取产品文档…'); setNotice('');
    try {
      if (type === 'reference') { const dataUrl = await prepareUpload(file); if (active(id)) patch({ reference: { name: file.name, dataUrl }, draft: {}, stage: 'idle', pending: undefined, error: undefined }); }
      else {
  ...
  async function generate(sync = false) {
    ...
    // When syncing, use the currently adopted Logo and copy, then refresh dependent designs.
    const initial = sync ? { ...studio, draft: { logo, copy: ctx.copyProject.finalPackage }, pending: undefined } : studio.stage === "completed" ? { ...studio, draft: {}, pending: undefined } : studio;
    try {
  ...
  return <div className="quick-studio">
    <header className="studio-heading"><div><div className="studio-eyebrow">PACKPILOT / DESIGN STUDIO</div><h1>{ready ? '让好设计，再进一步。' : '上传资料，让整套设计自然发生。'}</h1><p>{ready ? '点选想修改的地方，告诉 AI 你的想法。每一步都可以回看。' : '从瓶身参考与产品文档，生成配套 Logo、文案、瓶身与外包装。'}</p></div>{ready && <Link className="studio-primary" href="/workflow/6">质检与交付 ↗</Link>}</header>
    {(!ready || settingsOpen) && <section className="studio-setup">
      <div className="studio-upload-grid">
        <label className="studio-upload"><input type="file" accept="image/png,image/jpeg,image/webp" disabled={!!busy || !!studio.pending} onChange={e => { const file = e.target.files?.[0]; if (file) void upload(file, 'reference'); e.target.value = ''; }} />{studio.reference ? <img src={studio.reference.dataUrl} alt="瓶身参考" /> : <span className="upload-symbol">＋</span>}<strong>{studio.reference?.name || '上传瓶身参考图'}</strong><span>保留器型与比例，重新设计视觉</span><small>PNG / JPG / WebP · 最大 15MB</small></label>
        <label className="studio-upload"><input type="file" accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp" disabled={!!busy || !!studio.pending} onChange={e => { const file = e.target.files?.[0]; if (file) void upload(file, 'document'); e.target.value = ''; }} /><span className="upload-symbol">▤</span><strong>{studio.documentName || '上传产品文档'}</strong><span>自动提取品牌、卖点与产品信息</span><small>PDF / Word / TXT / 图片 · 最大 15MB</small></label>
      </div>
    ...
    {ready && <><div className="studio-result-bar"><div className="studio-tabs" role="tablist" aria-label="设计成果">{(['packaging', 'product', 'logo'] as const).map(k => <button role="tab" aria-selected={kind === k} className={kind === k ? 'active' : ''} disabled={!!busy || !!studio.edit} key={k} onClick={() => setKind(k)}>{assetNames[k]}</button>)}</div><button disabled={!!busy} onClick={() => setSettingsOpen(!settingsOpen)}>资料与生成设置</button></div>
      {activeAsset && !busy && <RegionEditor key={activeAsset.id} imageUrl={activeAsset.url} brandName={brief.brand.name} productName={brief.product.name} regions={studio.regions[activeAsset.id] || []} onRegions={regions => ctx.updateStudio(brief.projectId, s => ({ ...s, regions: { ...s.regions, [activeAsset.id]: regions } }))} pending={studio.edit?.assetId === activeAsset.id ? studio.edit.pending : undefined} onPending={pending => ctx.updateStudio(brief.projectId, s => ({ ...s, edit: pending ? { kind, assetId: activeAsset.id, pending } : undefined }))} onAdopt={(url, instruction) => { ctx.adoptStudioImage(brief.projectId, kind, url, instruction); setNotice(kind === 'packaging' ? '已采用修改，交付前会重新质检。' : '已采用修改。若需要将新 Logo / 产品设计应用到配套包装，可点击“同步整套设计”。'); }} />}
      <div className="studio-sync"><p>Logo 或瓶身有变化？将当前 Logo 和文案重新应用到瓶身与外包装。</p><button disabled={!!busy || !studio.reference} onClick={() => generate(true)}>同步整套设计</button></div>
