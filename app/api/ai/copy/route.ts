...
import { briefFillSystemPrompt, briefImportImageSystemPrompt, briefImportSystemPrompt, buildBriefFillPrompt, buildBriefImportImagePrompt, buildBriefImportPrompt } from "@/services/prompts/brief-import-prompt";
import { importDesignBrief } from "@/lib/import-design-brief";
import { countFieldSources, listMissingFillableFields, mergeAiFilledBrief, sourcesFromExtractedBrief } from "@/lib/brief-field-sources";
...
async function fillMissingBriefFields(extracted:DesignBrief,usage:{provider:string;durationMs:number;tokens?:number}){
 const missing=listMissingFillableFields(extracted);
 if(!missing.length)return{brief:extracted,fieldSources:sourcesFromExtractedBrief(extracted),...countFieldSources(sourcesFromExtractedBrief(extracted)),usage};
 const prompt=buildBriefFillPrompt(JSON.stringify(extracted),missing);
 try{
  const result=await chat([{role:"system",content:briefFillSystemPrompt},{role:"user",content:prompt}],"openlux","brief-import");
  const generated=importDesignBrief({...JSON.parse(cleanJson(result.data.choices?.[0]?.message?.content||"")) as Record<string,unknown>,projectId:extracted.projectId});
  const merged=mergeAiFilledBrief(extracted,generated);
  return{...merged,...countFieldSources(merged.fieldSources),usage:{...result.usage,tokens:(usage.tokens||0)+(result.usage.tokens||0),durationMs:(usage.durationMs||0)+(result.usage.durationMs||0)}};
 }catch{
  const fieldSources=sourcesFromExtractedBrief(extracted);
  return{brief:extracted,fieldSources,...countFieldSources(fieldSources),usage};
 }
}
