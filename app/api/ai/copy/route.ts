...
import { briefImportImageSystemPrompt, briefImportSystemPrompt, buildBriefImportImagePrompt, buildBriefImportPrompt } from "@/services/prompts/brief-import-prompt";
import { importDesignBrief } from "@/lib/import-design-brief";
import { countFieldSources, sourcesFromExtractedBrief } from "@/lib/brief-field-sources";
import { hasBriefContent } from "@/lib/design-content";
import { type CopyField, type CopyPackage } from "@/types/copy";
import { validatePackages } from "@/lib/copy-packages";
...
function parseImportedBrief(text:string,projectId:string){const raw=JSON.parse(cleanJson(text)) as Record<string,unknown>;const candidate=(raw.designBrief&&typeof raw.designBrief==="object"?raw.designBrief:raw) as Record<string,unknown>;const brief=importDesignBrief({...candidate,projectId});return brief;}
...
function isImageDataUrl(value:string){return /^data:image\/(png|jpe?g|webp|gif);base64,/i.test(value);}
async function chat(messages:ChatMessage[],provider:"deepseek"|"gemini"|"openlux",generator="copy"){
 if(provider==="openlux"){
  const base=aiServerConfig.openlux.baseUrl.replace(/\/$/,"");
  return fetchAiJson<DeepSeekPayload>({url:`${base}/chat/completions`,apiKey:aiServerConfig.openlux.apiKey,provider:"openlux",generator,timeoutMs:90000,body:{model:aiServerConfig.openlux.model,messages,temperature:.2,response_format:{type:"json_object"},max_tokens:5000}});
 }
 if(provider==="gemini"){
...
export async function POST(request:Request){
 ...
 try{const body=await request.json();if(body.action==="brief-import"){if(!aiServerConfig.openlux.apiKey)return Response.json({error:"文档解析模型（OpenLux gpt-5.6-luna）密钥未配置"},{status:503});const params=body.params as {documentText?:string;projectId:string;fileName:string;images?:{name?:string;dataUrl?:string}[]};const images=(Array.isArray(params.images)?params.images:[]).map(item=>({name:String(item.name||"image"),dataUrl:String(item.dataUrl||"")})).filter(item=>isImageDataUrl(item.dataUrl)).slice(0,6);if(images.some(item=>item.dataUrl.length>12_000_000))return Response.json({error:"单张图片过大，请压缩后重试"},{status:400});if(!params.documentText?.trim()&&!images.length)return Response.json({error:images.length===0&&Array.isArray(params.images)?"图片格式无效，请上传 JPG / PNG / WEBP":"文档文字为空"},{status:400});const system=images.length?briefImportImageSystemPrompt:briefImportSystemPrompt;const prompt=images.length?buildBriefImportImagePrompt(params.projectId,params.fileName,images.length):buildBriefImportPrompt(params.documentText||"",params.projectId,params.fileName);const userContent=images.length?[{type:"text" as const,text:prompt},...images.map(image=>({type:"image_url" as const,image_url:{url:image.dataUrl}}))]:prompt;const first=await chat([{role:"system",content:system},{role:"user",content:userContent}],"openlux","brief-import");const content=first.data.choices?.[0]?.message?.content||"";let brief;let usage=first.usage;try{brief=parseImportedBrief(content,params.projectId);}catch(error){const repair=await chat([{role:"system",content:system},{role:"user",content:userContent},{role:"assistant",content},{role:"user",content:`上次结果未通过校验：${error instanceof Error?error.message:"JSON 格式错误"}。请只依据原文或图中可见信息，保留已提取的信息，其他可见资料放 additionalInfo；未提供的信息留空，不要求任何具体字段，重新返回严格 JSON。`}],"openlux","brief-import");usage={...repair.usage,tokens:(first.usage.tokens||0)+(repair.usage.tokens||0)};brief=parseImportedBrief(repair.data.choices?.[0]?.message?.content||"",params.projectId);}if(!hasBriefContent(brief)&&params.documentText?.trim())brief.additionalInfo=params.documentText.trim();const fieldSources=sourcesFromExtractedBrief(brief);return Response.json({data:{brief,fieldSources,...countFieldSources(fieldSources)},usage});}const provider=body.provider==="gemini"?"gemini":"deepseek";if(provider==="deepseek"&&!aiServerConfig.deepseek.apiKey)return Response.json({error:"DeepSeek 密钥未配置"},{status:503});if(provider==="gemini"&&!aiServerConfig.yunwu.apiKey)return Response.json({error:"Gemini 兼容服务密钥未配置"},{status:503});if(body.action==="structure-recommend"){const params=body.params as {brief:Parameters<typeof buildProductStructurePrompt>[0];count?:number};const prompt=buildProductStructurePrompt(params.brief,params.count);const first=await chat([{role:"system",content:productStructureSystemPrompt},{role:"user",content:prompt}],provider);const content=first.data.choices?.[0]?.message?.content||"";let recommendations;let usage=first.usage;try{recommendations=parseStructureRecommendations(content);}catch(error){const repair=await chat([{role:"system",content:productStructureSystemPrompt},{role:"user",content:prompt},{role:"assistant",content},{role:"user",content:`上次输出未通过校验：${error instanceof Error?error.message:"JSON 格式错误"}。请重新返回完整严格 JSON。`}],provider);usage={...repair.usage,tokens:(first.usage.tokens||0)+(repair.usage.tokens||0)};recommendations=parseStructureRecommendations(repair.data.choices?.[0]?.message?.content||"");}return Response.json({data:recommendations,usage});}if(body.action==="rewrite"){const prompt=buildRewritePrompt(body.params);const result=await chat([{role:"system",content:copySystemPrompt},{role:"user",content:prompt}],provider);const content=result.data.choices?.[0]?.message?.content||"";const parsed=JSON.parse(cleanJson(content)) as {alternatives?:string[]};if(!Array.isArray(parsed.alternatives)||parsed.alternatives.length<3)throw new Error("重写结果格式不正确");return Response.json({data:parsed.alternatives.slice(0,3),usage:result.usage});}
 const params=body.params;const validIds=params.brief.insights.map((item:{id:string})=>item.id);const painIds=params.brief.insights.filter((item:{type:string})=>item.type==="pain_point").map((item:{id:string})=>item.id);const prompt=buildCopyGenerationPrompt(params);const first=await chat([{role:"system",content:copySystemPrompt},{role:"user",content:prompt}],provider);const content=first.data.choices?.[0]?.message?.content||"";let packages:CopyPackage[];let usage=first.usage;try{packages=validatePackages(content,validIds,painIds,params.baseCopyId,params.allowPartial===true);}catch(error){const repair=await chat([{role:"system",content:copySystemPrompt},{role:"user",content:prompt},{role:"assistant",content},{role:"user",content:`上次输出未通过校验：${error instanceof Error?error.message:"格式错误"}。请删除所有未由 Brief 提供的百分比、浓度、周期、测试和实验数据；${params.allowPartial?"只返回当前方案，缺失内容留空，不要求凑齐六项文案或覆盖全部痛点":"确保全部痛点 id 在三套方案中至少各被一条文案直接关联"}，然后重新输出严格 JSON。`}],provider);usage={...repair.usage,tokens:(first.usage.tokens||0)+(repair.usage.tokens||0)};packages=validatePackages(repair.data.choices?.[0]?.message?.content||"",validIds,painIds,params.baseCopyId,params.allowPartial===true);}return Response.json({data:packages,usage});
