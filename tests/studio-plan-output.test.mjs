...
test('invalid typography reports the field and distinguishes missing, empty, type and overlength without echoing content',()=>{
  for(const [value,code] of [[undefined,'missing'],['  ','empty'],[{},'empty'],[false,'invalid_type'],['敏'.repeat(2401),'too_long']]) {
    assert.throws(()=>parseDesignPlan({...plan,typography:value}),error=>{
      assert.equal(error.diagnostic.field,'typography');assert.equal(error.diagnostic.reason,code);
      assert.match(error.message,/字体和信息层级/);assert.doesNotMatch(error.message,/敏敏/);return true;
    });
  }
});
test('compatible nested plan requires no second paid call and logs only shape metadata',async t=>{
  ...
  assert.ok(schema.required.includes('typography'));assert.equal(schema.properties.typography.type,'string');
...
test('one malformed model output is repaired within the same job and accounts for both text calls',async t=>{
  const f=adapter(t,[{plan:{...plan,typography:null}},{plan}]);
  ...
  assert.equal(f.calls[1].contents.at(-1).role,'user');assert.match(f.calls[1].contents.at(-1).parts[0].text,/typography/);
  assert.match(JSON.stringify(f.logs),/missing/);assert.ok(f.logs.every(args=>JSON.stringify(args).includes('test-plan-job')));
});
...
test('repeated invalid plan fails clearly after two calls without a fabricated default or sensitive output',async t=>{
  const f=adapter(t,[{plan:{...plan,typography:'private-'.repeat(400)}}]);
  await assert.rejects(f.run(),error=>{assert.match(error.message,/typography|字体和信息层级/);assert.match(error.message,/2400/);assert.match(error.message,/test-plan-job/);assert.doesNotMatch(error.message,/private-private/);return true});
