...
test('durable metadata-only retry retains UUID and verified user; actual attempts differ', async () => {
  ...
  const reporter=createUsageReporter({tool:'chanpinsheji',getMainAppUrl:()=> 'https://main.test', secret:()=> 'test-secret',outboxDir:()=>dir,fetchImpl:async (_url,init)=>{
    const event=JSON.parse(init.body); requests.push(event);
    assert.equal(init.headers['x-usage-tool'],'chanpinsheji');
...
test('only an explicit successful JSON acknowledgement removes durable reports', async () => {
  ...
  const reporter=createUsageReporter({tool:'chanpinsheji',getMainAppUrl:()=> 'https://main.test',secret:()=> 'test',outboxDir:()=>directory,fetchImpl:async()=>acknowledgement==='html'?new Response('<html>login</html>'):Response.json({success:acknowledgement==='success'})});
  try {
