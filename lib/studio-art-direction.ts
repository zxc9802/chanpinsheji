...
export type StudioCreativeRequest = Context & (
  ...
  { action: 'review'; kind: AssetKind; plan: DesignPlan; original: string; revised?: string; references: string[] }
);
...
export function validateCreativeRequest(value: unknown): StudioCreativeRequest {
  ...
  return { action: v.action, brief: v.brief, styleHint: v.styleHint, referenceMode: v.referenceMode, kind: v.kind, plan: parseDesignPlan(v.plan), original: validateRegionImage(v.original), ...(v.revised ? { revised: validateRegionImage(v.revised) } : {}), references: v.references.map(validateRegionImage) };
}
