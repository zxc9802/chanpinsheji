...
function upstreamError(response:Response,raw:string){const detail=raw.replace(/\s+/g," ").trim().slice(0,300);return new Error(`AI 上游返回 HTTP ${response.status}${detail?`：${detail}`:""}`);}
export async function fetchAiJson<T>(args: { url: string; apiKey: string; body: unknown; timeoutMs: number; provider: string; generator: string; authHeaders?: Record<string,string> }): Promise<{ data: T; usage: ServerAiUsage }> {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      const raw=await response.text();let payload:T & { error?: { message?: string } | string; usage?: { prompt_tokens?: number; completion_tokens?: number; total_tokens?: number; generated_images?: number; output_tokens?: number } };
      try{payload=JSON.parse(raw) as typeof payload;}catch{throw upstreamError(response,raw);}
